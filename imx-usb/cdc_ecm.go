...
const MaxPacketSize uint16 = 512

func (ecm *ECM) addControlInterface() {
	device := ecm.Device
	...
	iMacAddress, _ := device.AddString(strings.ReplaceAll(ecm.HostMAC.String(), ":", ""))
...
func (ecm *ECM) addDataInterfaces() {
	device := ecm.Device
...
func (ecm *ECM) configureDevice() {
	...
	iSerial, _ := device.AddString(ecm.DeviceMAC.String())
